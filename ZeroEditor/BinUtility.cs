...
public static class BinUtil
{
    ...
    public static void AssertInBounds(long offset, int size, long fileSize, string what)
    {
        if (offset < 0 || offset + size > fileSize)
            throw new InvalidDataException($"{what} out of bounds: off=0x{offset:X}, size=0x{size:X}, file=0x{fileSize:X}");
    }
    ...
    public static uint ReadU32At(BinaryReader br, long offset)
    {
        var s = br.BaseStream;
        long prev = s.Position;
        s.Position = offset;
        try { return br.ReadUInt32(); }
        finally { s.Position = prev; }
    }

    public static ushort ReadU16At(BinaryReader br, long offset)
    {
        var s = br.BaseStream;
        long prev = s.Position;
        s.Position = offset;
        try { return br.ReadUInt16(); }
        finally { s.Position = prev; }
    }

    public static byte ReadU8At(BinaryReader br, long offset)
    {
        var s = br.BaseStream;
        long prev = s.Position;
        s.Position = offset;
        try { return br.ReadByte(); }
        finally { s.Position = prev; }
    }
}
