...
static class NativeDark
{
    ...
    public static void ApplyDarkExplorerThemeDeep(IntPtr hwnd)
    {
        if (hwnd == IntPtr.Zero)
            return;
...
public static class ThemeManager
{
    ...
    static void ApplyScrollbarThemeNowOrOnHandle(Control c)
    {
        void apply()
        {
            NativeDark.ApplyDarkExplorerThemeDeep(c.Handle);
            var form = c.FindForm();
    ...
    static void ApplyPerControlOverrides(Control c)
    {
        switch (c)
        {
            ...
            case TabControl tc when tc is not DarkTabControl:
                tc.BackColor = DarkPalette.Window;
                tc.ForeColor = DarkPalette.Fore;
                foreach (TabPage tp2 in tc.TabPages)
                {
                    tp2.UseVisualStyleBackColor = false;
                    tp2.BackColor = DarkPalette.Window;
                    tp2.ForeColor = DarkPalette.Fore;
                }
                break;
            ...
            case DataGridView dgv:
                ...
                dgv.DataBindingComplete += (_, __) =>
                {
                    StyleDataGridViewColumns(dgv);
                    if (dgv.IsHandleCreated)
                        NativeDark.ApplyDarkExplorerThemeDeep(dgv.Handle);
                };
                StyleDataGridViewColumns(dgv);
                dgv.EditingControlShowing += (_, e) =>
                {
                    ...
                    {
                        tb2.BackColor = DarkPalette.Sunken;
                        tb2.ForeColor = DarkPalette.Fore;
                        tb2.BorderStyle = BorderStyle.FixedSingle;
                    }
                    else if (e.Control is ComboBox cb2)
                    {
                        cb2.FlatStyle = FlatStyle.Flat;
                        cb2.BackColor = DarkPalette.Sunken;
                        cb2.ForeColor = DarkPalette.Fore;
                    }
                };
