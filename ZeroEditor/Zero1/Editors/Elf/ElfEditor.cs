...
namespace ZeroEditor.Zero1.Editors
{
    public partial class Zero1ElfEditor : UserControl
    {
        ...
        private Segment[] _FuyuSegments = Array.Empty<Segment>();
        private readonly List<List<ENE_DAT>> _FuyuData = new();
        ...
        public void LoadElf(string path, GameContext ctx)
        {
            ...
            _JibakuData.Clear();
            _JibakuData.AddRange(ReadBySegments<ENE_DAT>(_br!, _JibakuSegments));

            _FuyuSegments = elf.FuyuByNight ?? Array.Empty<Segment>();
            _FuyuData.Clear();
            _FuyuData.AddRange(ReadBySegments<ENE_DAT>(_br!, _FuyuSegments));
        ...
        private void listBoxData_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            switch (listBoxData.SelectedItem?.ToString())
            {
                case "Jibaku Enemy Data":
                    ShowEditor(new EneDatEditor(_JibakuData));
                    break;

                case "Fuyu Enemy Data":
                    ShowEditor(new EneDatEditor(_FuyuData));
                    break;

                case "Auto Enemy Data":
                    break;

                case "Fog Data":
                    ShowEditor(new FogEditor(_fogParams, _fogRgbs, _fogParamFinder, _fogRgbFinder));
                    break;

                case "Map Item Data":
                    ShowEditor(new MapItemEditor(_mapItems));
                    break;
            }
        }
        ...
        private void SaveElf()
        {
            ...
            WriteBySegments(_bw, _JibakuSegments, _JibakuData);
            WriteBySegments(_bw, _FuyuSegments, _FuyuData);
        ...
        private static List<List<T>> ReadBySegments<T>(BinaryReader br, Segment[] segments) where T : struct
        {
            var result = new List<List<T>>(segments.Length);
            foreach (var seg in segments)
            {
                if (seg.Count <= 0 || seg.Offset == 0)
                {
                    result.Add(new List<T>());
                    continue;
                }
                result.Add(BinUtil.ReadArrayAt<T>(br, seg));
            }
            return result;
        }

        private static void WriteBySegments<T>(BinaryWriter bw, Segment[] segments, List<List<T>> data) where T : struct
        {
            if (segments.Length != data.Count)
                throw new InvalidOperationException($"Segment count mismatch writing {typeof(T).Name}: segments={segments.Length}, data={data.Count}");
            ...
            for (int i = 0; i < segments.Length; i++)
            {
                ...
                BinUtil.WriteArrayAt(bw, seg, data[i]);
            }
        }
    }
}
