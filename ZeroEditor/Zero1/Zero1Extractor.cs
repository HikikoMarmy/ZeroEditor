...
namespace ZeroEditor
{
    public sealed class Zero1Extractor : IGameExtractor
    {
            ...
            CancellationToken ct)
        {
            ...
            int expectedTblLen = checked(count * 8);
            ...
            TryUpdateManifestFlags(manifestPath, count, existsFlags, cmpFlags);
        }
        ...
        private static async Task TryExplodePk2Async(string pk2Path, string filesRoot, IProgress<string>? log, CancellationToken ct)
        {
            byte[] buffer;
            try
            {
            ...
            {
                log?.Report($"PK2: could not read ({ex.Message}), kept as file: {Path.GetRelativePath(filesRoot, pk2Path)}");
                return;
            }
            ...
            {
                log?.Report($"PK2: failed to explode ({ex.Message}), kept as file: {Path.GetRelativePath(filesRoot, pk2Path)}");
            }
        }
