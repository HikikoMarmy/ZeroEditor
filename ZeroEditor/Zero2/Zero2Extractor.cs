...
namespace ZeroEditor
{
    public sealed class Zero2Extractor : IGameExtractor
    {
            ...
            CancellationToken ct)
        {
            ...
            if (WriteDebugImgMap)
            {
                mapPath = Path.Combine(outputProjectRoot, "img_map_original.csv");
            ...
            if (WriteDebugImgMap && mapPath != null && mapSb != null)
            {
                ...
                log?.Report($"FF2 original map: {Path.GetRelativePath(outputProjectRoot, mapPath)}");
            }

            TryUpdateManifestFlags(manifestPath, layout.Count, existsFlags, cmpFlags);
            ...
            for (int i = 0; i < layout.Count; i++)
            {
                ...
                var outPath = Path.Combine(filesRoot, rel.Replace('/', Path.DirectorySeparatorChar));
                ...
                {
                    await TryExplodePk2Async(outPath, filesRoot, log, ct);
                }
                else if (outPath.EndsWith(".pak", StringComparison.OrdinalIgnoreCase))
                {
                    await TryExplodePakAsync(outPath, filesRoot, log, ct);
                }
            }
        }
            ...
            CancellationToken ct)
        {
            ...
            try
            {
                ...
                log?.Report(
                    $"PK2: exploded -> {Path.GetRelativePath(filesRoot, workDir)}/ " +
                    $"(from {Path.GetRelativePath(filesRoot, pk2Path)})");
            }
            catch (Exception ex)
            {
                log?.Report(
            ...
            try
            {
                ...
                log?.Report(
                    $"PK2: deleted original archive after explode: {Path.GetRelativePath(filesRoot, pk2Path)}");
            }
            catch (Exception ex)
            {
                log?.Report(
                    $"PK2: exploded but could not delete original ({ex.Message}), file left in place: {Path.GetRelativePath(filesRoot, pk2Path)}");
            }
        }
            ...
            CancellationToken ct)
        {
            ...
            if (written > 0)
                log?.Report($"PAK: exploded -> {Path.GetRelativePath(filesRoot, root)}/ ({written} files)");
            ...
            {
                log?.Report(
                    $"PAK: exploded but could not delete original ({ex.Message}), file left in place: {Path.GetRelativePath(filesRoot, pakPath)}");
            }
        }
