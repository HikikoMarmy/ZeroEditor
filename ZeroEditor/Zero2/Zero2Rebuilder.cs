...
namespace ZeroEditor
{
    public sealed class Zero2Rebuilder : IGameRebuilder
    {
            ...
            CancellationToken ct)
        {
            ...
            var elfPath = Path.Combine(filesRoot, ctx.ElfName);
            if (!File.Exists(elfPath))
                throw new FileNotFoundException("ELF not found in files folder.", elfPath);
            ...
            CancellationToken ct = default)
        {
            ...
            if (fileTable.Count < imgCount)
                throw new InvalidDataException($"file_table has {fileTable.Count}, expected ≥ IMG entries {imgCount}.");
            ...
            if (originalEntries.Count != imgCount)
                throw new InvalidDataException($"Original IMG layout entries ({originalEntries.Count}) != expected IMG count ({imgCount}).");
            ...
            for (int phys = 0; phys < physicalIndexOrder.Count; phys++)
            {
                ...
                {
                    string? pk2Folder = FindPk2FolderForRel(filesRoot, relPath);
                ...
                {
                    string? pakFolder = FindPakFolderForRel(filesRoot, relPath);
                ...
                if ((plainBytes == null) && !File.Exists(srcPath))
                {
                    ...
                    log?.Report($"[phys {phys + 1}/{physicalIndexOrder.Count}] idx={idx} {relPath}  (missing, exists={exists})  size=0  start_sec={startSector}");
                ...
                {
                    log?.Report($"[phys {phys + 1}/{physicalIndexOrder.Count}] idx={idx} {relPath}  (zero-byte file, exists={exists}) start_sec={startSector}");
                ...
                if (plainBytes == null)
                {
                    ...
                    log?.Report($"[phys {phys + 1}/{physicalIndexOrder.Count}] idx={idx} {relPath}  (no data, treated as missing)  start_sec={startSector}");
                    entries[idx] = (startSector, 0u, orig.CmpSize, orig.Compressed, exists, relPath);
                    continue;
                }
            ...
            IProgress<string>? log)
        {
            ...
            var baseIsoPath = ProjectLayout.FromProjectRoot(projectRoot).BaseIsoPath;
            if (!File.Exists(baseIsoPath))
                throw new FileNotFoundException("Base ISO not found (expected _base.iso).", baseIsoPath);
            ...
            if (read != tbl.Length)
                throw new InvalidDataException($"Base ELF cd table size mismatch: expected {tbl.Length} bytes, got {read}.");
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                else
                {
                    ...
                    if (!exists && origExists)
                    {
                        ...
                        log?.Report($"FF2 ELF: entry {i} '{rel}' was present in original table (size=0x{origSizeField:X}, cmpSize=0x{origCmpSizeField:X}, cmp={(origCmp ? 1 : 0)}) but is missing in rebuilt IMG_BD (exists=0).");
                    }
        ...
        private static string? FindPk2FolderForRel(string filesRoot, string rel)
        {
            var relOS = rel.Replace('/', Path.DirectorySeparatorChar);
        ...
        private static string? FindPakFolderForRel(string filesRoot, string rel)
        {
            var relOS = rel.Replace('/', Path.DirectorySeparatorChar);
            var dir = Path.GetDirectoryName(relOS) ?? string.Empty;
            var stem = Path.GetFileNameWithoutExtension(relOS);
